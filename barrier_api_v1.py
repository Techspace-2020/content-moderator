...
def analyze_message(input: MessageInput):
    try:
        ...
        classification_key = make_cache_key("classification", input.message)
        classification_result = cache.get(classification_key)
        if not classification_result:
            ...
            cache.set(
                classification_key, classification_result, expire=CACHE_EXPIRY_SECONDS
            )
        ...
        if classification_result["classification"] == "Claim/News/Article/Statement":
            # Use the original input message as input to the fake_news_agent
            factcheck_key = make_cache_key("factcheck", input.message)
            factcheck_result = cache.get(factcheck_key)
            if factcheck_result:
            ...
            if isinstance(response.content, str):
                try:
                    ...
                    cache.set(
                        factcheck_key, response_content, expire=CACHE_EXPIRY_SECONDS
                    )
                    return {"type": "json", "data": response_content}
                except json.JSONDecodeError:
                    return {"type": "text", "data": response.content.strip()}
            else:
                cache.set(factcheck_key, response.content, expire=CACHE_EXPIRY_SECONDS)
